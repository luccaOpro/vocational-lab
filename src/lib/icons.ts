/*
  icons.ts — fuente única de los iconos lineales del aula.
  ---
  Los paths vienen de Lucide (MIT). Tanto el componente <Icon /> como
  los renders dinámicos por innerHTML los toman desde acá para no
  duplicar SVG en el código.

  Uso desde innerHTML (string):
    import { iconHtml } from "../lib/icons";
    $el.innerHTML = `${iconHtml("inbox")} <span>Sin solicitudes</span>`;

  Uso desde Astro (componente):
    import Icon from "../components/aula/Icon.astro";
    <Icon name="inbox" />
*/
...
/**
 * Devuelve el SVG completo como string, listo para inyectar en innerHTML.
 * Hereda el color del contenedor (stroke=currentColor).
 *
 * @param name nombre del icono
 * @param size lado en px (default 24)
 */
export function iconHtml(name: IconName, size = 24): string {
